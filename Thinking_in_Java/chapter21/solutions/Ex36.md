...
*///
```
练习是模糊不清的，允许许多方法。我们的解决方案工作得很好，但试着尝试其他方法。例如，为了提高餐厅的生产力，消除重复往返的低效，尽快交付每个盘子，也许可以分组食物类型的交付。 

Table类创建一组订单。每个客户在一个Table上点菜，当他们都完成时，Table将订单票传递给一个服务员。CyclicBarrier同步任务。 

OrderTicket包括许多订单，每个厨师处理一张票。

Restaurant产生Table任务，所以顾客成群结队，占据一张特定的桌子。 
