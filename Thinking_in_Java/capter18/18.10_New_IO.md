...
## 转换数据
在GetChannel.java程序中，为了输出文件中的信息，我们***必须***每次只读去一个字节的数据，然后让其强制转换成char类型。这种方法似乎有点原始——如果查看一下java.nio.CharBuffer类，
将会发现有一个toString()方法是这样定义的：“返回一个包含缓冲器中所有字符的字符串”，并且ByteBuffer有一个asCharBuffer()方法可以获得CharBuffer对象，那么为什么不用它？
...
那么一个ByteBuffer足可以容纳12个字符，但是"Some text"只有9个字符，剩余的内容为零的字节仍会出现在由toString()所产生的CharBuffer的表示中，我们可以在输出结果中看到。
