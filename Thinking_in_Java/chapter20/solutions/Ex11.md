...
annotations.Ex11_TestNoteDemo
  . initialization  : Tests if the new HashSet is empty 
  . _containts  : Tests the HashSet.contains(Object o) method 
  . _remove  : Tests the HashSet.remove(Object o) method 
  . _clear  : Test the HashSet.clear() method 
OK (4 tests)
*///
```
可以看到，在测试方法上增加了@TestNote注解的，都会在测试时显示其附加的信息。

注意：我们在Ex11_AtUnit2.java程序中设定，@Test和@TestNote两个注解必须同时出现。
